...
def _run_one_question_openrouter(api_key: _SecretValue) -> str:
    model_candidates = [
        os.getenv("REPTRACE_OPENROUTER_TEST_MODEL", "").strip() or "openai/gpt-3.5-turbo",
        "openai/gpt-4o-mini",
    ...
    for model_name in model_candidates:
        try:
            ...
            # Reasoning models generally need more max_tokens for final answer + reasoning traces.
            max_length = 200 if "pony-alpha" in model_name else 32
            response = wrapper.generate(
                prompt,
